...
import { useGetTmdbGenres, useDiscoverTmdb, GetTmdbGenresType, DiscoverTmdbType, getGetTmdbGenresQueryKey, getDiscoverTmdbQueryKey, useSearchTmdb, getSearchTmdbQueryKey } from "@workspace/api-client-react";
import { Compass, Loader2, Play, Search, X } from "lucide-react";
...
export default function DiscoverPage() {
  ...
  const { data: discoverData, isLoading: discoverLoading } = useDiscoverTmdb(
    ...
    { query: { enabled: !searchQuery, queryKey: getDiscoverTmdbQueryKey({ type: type as DiscoverTmdbType, genre: selectedGenre, page: 1 }) } }
  );
  const { data: searchData, isLoading: searchLoading } = useSearchTmdb(
    { query: searchQuery, page: 1 },
    { query: { enabled: !!searchQuery, queryKey: getSearchTmdbQueryKey({ query: searchQuery, page: 1 }) } }
  );

  const genres = genresData?.genres || [];
  const items = searchQuery ? (searchData?.results || []) : (discoverData?.results || []);
      ...
      <div className="max-w-6xl mx-auto pt-14 md:pt-4 w-full flex-1 flex flex-col">
        ...
        <form onSubmit={handleSearch} className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 rounded-xl bg-card border border-border text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:border-primary/60 transition-colors"
          />
          ...
          <>
            ...
            <div className="flex bg-muted rounded-lg p-1 w-fit mb-4">
              <button
                ...
                className={cn("px-5 py-1.5 rounded-md text-sm font-medium transition-all", type === GetTmdbGenresType.movie ? "bg-card text-foreground shadow" : "text-muted-foreground")}
              >Movies</button>
              <button
                onClick={() => { setType(GetTmdbGenresType.tv); setSelectedGenre(undefined); }}
                className={cn("px-5 py-1.5 rounded-md text-sm font-medium transition-all", type === GetTmdbGenresType.tv ? "bg-card text-foreground shadow" : "text-muted-foreground")}
              >TV Series</button>
            </div>

            {/* Genre Pills — horizontal scroll */}
            <div className="overflow-x-auto no-scrollbar mb-6">
              <div className="flex gap-2 whitespace-nowrap w-max pb-1">
                <button
        ...
        {isLoading ? (
              ...
              return (
                <div key={item.id} className="group relative rounded-xl overflow-hidden bg-card aspect-[2/3] border border-border/50 hover:border-primary/50 transition-colors">
                  {poster ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-muted text-muted-foreground p-4 text-center text-xs">No Image</div>
                  )}
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-end pb-3 backdrop-blur-sm">
                    <Play className="w-9 h-9 text-white fill-white mb-2" />
          ...
          <div className="flex-1 flex items-center justify-center text-muted-foreground py-20 text-center">
            <p>{searchQuery ? `No results for "${searchQuery}"` : "No results found for this genre."}</p>
          </div>
        )}
      </div>
    </div>
