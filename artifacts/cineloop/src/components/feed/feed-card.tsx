...
export function FeedCard({
  ...
  const mediaType = isMovie
    ? GetTmdbVideosType.movie
    : GetTmdbVideosType.tv;
  ...
  const { data: videos } = useGetTmdbVideos(
    { id: item.id, type: mediaType },
    {
      query: {
        enabled: isActive,
        queryKey: getGetTmdbVideosQueryKey({
          id: item.id,
          type: mediaType,
        }),
      },
  ...
  const trailer =
    ...
    videos?.results?.find(
      (v) => v.type === "Teaser" && v.site === "YouTube" && v.key
    ) ||
    videos?.results?.find(
  ...
  // remove cards with no playable video
  ...
  const fadeVolumeIn = useCallback(() => {
    ...
    for (let i = 1; i <= STEPS; i++) {
      setTimeout(() => {
        sendYtCommand(iframeRef.current, "setVolume", [
          Math.round((i / STEPS) * 85),
        ]);
      }, (i / STEPS) * DURATION_MS);
    }
  }, []);
  ...
  const iframeSrc = `https://www.youtube.com/embed/${trailer.key}?autoplay=1&mute=1&controls=0&rel=0&loop=1&playsinline=1&playlist=${trailer.key}&enablejsapi=1&origin=${safeOrigin}`;
